...
final class ConfigEditorStore {
    ...
    func specName(_ specId: Int) -> String {
        guard let classId = selectedClassId, let name = ClassNames.specName(classId: classId, specId: specId) else { return String(localized: "专精\(specId)") }
        return name
    }
    ...
    func load(classId: Int) {
        ...
        do {
            ...
            status = String(localized: doc.isModernFormat ? "可编辑" : "此文件仍是旧版稀疏索引格式，请先迁移到 states/auras/spells/items/group 后再编辑。")
    ...
    func addSpellFromDatabase(spellId: Int64, name: String) -> String? {
        guard var doc = document else { return String(localized: "请先选择一个职业文件。") }
        guard doc.isModernFormat else { return String(localized: "旧版稀疏索引格式暂不支持添加技能。") }
        if let error = validateSpellsList() { return error }
        if doc.spellsList.contains(where: { $0.spellId == spellId }) { return String(localized: "已有此技能：\(name)（\(spellId)）") }
        let used = Set(doc.spellsList.map(\.index).filter { (1...100).contains($0) })
        guard let next = (1...100).first(where: { !used.contains($0) }) else { return String(localized: "索引 1–100 已全部使用，无法继续添加技能。") }
    ...
    func addItemFromDatabase(itemId: Int64, name: String) -> String? {
        guard var doc = document else { return String(localized: "请先选择一个职业文件。") }
        guard doc.isModernFormat else { return String(localized: "旧版稀疏索引格式暂不支持添加物品。") }
        if let error = validateItemsList() { return error }
        if doc.itemsList.contains(where: { $0.itemId == itemId }) { return String(localized: "已有此物品：\(name)（\(itemId)）") }
    ...
    func validateSpellsList() -> String? {
        ...
        for (i, e) in edited.enumerated() {
            if e.spellId <= 0 { return String(localized: "技能列表第 \(i + 1) 行的法术 ID 必须是正整数。") }
            if !(1...100).contains(e.index) { return String(localized: "技能列表第 \(i + 1) 行的索引必须是 1–100 的整数。") }
            if e.name.isBlank { return String(localized: "技能列表第 \(i + 1) 行的名称不能为空。") }
            if !ids.insert(e.spellId).inserted { return String(localized: "技能列表中的法术 ID \(e.spellId) 重复。") }
        }
        ...
        if let conflict = edited.first(where: { hidden.contains($0.spellId) })?.spellId {
            return String(localized: "法术 ID \(conflict) 已被技能列表中索引 101+ 的条目使用。")
        }
        return nil
    }
    ...
    func validateItemsList() -> String? {
        ...
        for (i, e) in doc.itemsList.enumerated() {
            if e.itemId <= 0 { return String(localized: "物品列表第 \(i + 1) 行 itemId 必须是正整数。") }
            if e.index <= 0 { return String(localized: "物品列表第 \(i + 1) 行的索引必须是正整数。") }
            if e.name.isBlank { return String(localized: "物品列表第 \(i + 1) 行名称不能为空。") }
            if !ids.insert(e.itemId).inserted { return String(localized: "物品列表 itemId \(e.itemId) 重复。") }
            if !indices.insert(e.index).inserted { return String(localized: "物品列表索引 \(e.index) 重复。") }
        }
        return nil
    }
    ...
    func validateItems() -> String? {
        ...
        for specId in doc.specs.keys.sorted() {
            ...
            for (i, item) in spec.items.enumerated() {
                guard let id = item.itemId, id > 0 else { return String(localized: "专精 \(specId) 的物品第 \(i + 1) 行 itemId 必须是正整数。") }
                if item.name.isBlank { return String(localized: "专精 \(specId) 的物品第 \(i + 1) 行名称不能为空。") }
                if !ids.insert(id).inserted { return String(localized: "专精 \(specId) 的物品 itemId \(id) 重复。") }
                if !names.insert(item.name).inserted { return String(localized: "专精 \(specId) 的物品名称「\(item.name)」重复。") }
                if reserved.contains(item.name) { return String(localized: "专精 \(specId) 的物品名称「\(item.name)」与状态、特殊、能量或配置开关字段重名。") }
            }
        }
        return nil
    }
    ...
    func save() {
        ...
        guard doc.isModernFormat else { errorMessage = String(localized: "此文件仍是旧版稀疏索引格式，无法用图形编辑器保存。"); return }
        ...
        Task {
            do {
                ...
                let outcome = await model.afterLuaSaved(relativePath: relative, classId: classId)
                status = String(localized: outcome.hasIssue ? "本地已保存并更新配置，但游戏同步失败" : "已保存配置及该职业的模块")
