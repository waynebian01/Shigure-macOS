            ...
            VStack(alignment: .leading, spacing: 16) {
                ForEach(ReferenceData.bossGroups) { group in
                    if !group.title.isEmpty {
                        Text(group.title).font(.title3.bold()).padding(.top, 8)
                    } else {
        ...
        ScrollView {
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 380), spacing: 12)], alignment: .leading, spacing: 12) {
                ForEach(ReferenceData.commonFieldCards, id: \.title) { card in
                    VStack(alignment: .leading, spacing: 6) {
                        Text(card.title).font(.headline).foregroundStyle(Color.accentColor)
...
struct AboutPage: View {
            ...
            VStack(alignment: .leading, spacing: 16) {
                HStack(alignment: .top, spacing: 20) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Text(ReferenceData.productTagline).foregroundStyle(.secondary)
                        Grid(alignment: .leading, horizontalSpacing: 16, verticalSpacing: 6) {
                            ...
                            row("游戏插件目录", model.addOnsDirectory?.path ?? "未检测到游戏")
                        }
                        HStack {
    ...
    private func article(_ title: String, _ body: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Label(title, systemImage: "scalemass").font(.headline)
