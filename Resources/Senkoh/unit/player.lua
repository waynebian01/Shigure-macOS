...
local spellsList = Senkoh.spellsList

local drinkStatusTimer = nil

function Senkoh:InitializeCharacterState()
...
function Senkoh:InitializeSpecializationState()
...
function Senkoh:RebuildSpecializationState()
...
function Senkoh:RefreshPlayerValidity()
...
function Senkoh:RefreshPlayerCombatState()
    state.combat = UnitAffectingCombat("player")
end

function Senkoh:RefreshPlayerCombatDuration()
...
function Senkoh:SetPlayerMoving(isMoving)
...
function Senkoh:RefreshPlayerCastStateBlocks()
...
function Senkoh:RefreshPlayerCastingStateBlocks()
    self:UpdateStateBlock("状态", "施法(正计时)")
    self:UpdateStateBlock("状态", "施法(倒计时)")
end

function Senkoh:RefreshPlayerChannelStateBlock()
    self:UpdateStateBlock("状态", "引导")
end

function Senkoh:RefreshPlayerEmpowerStateBlocks()
    self:UpdateStateBlock("状态", "蓄力")
    self:UpdateStateBlock("状态", "蓄力层数")
end

function Senkoh:UpdatePlayerHealth()
...
function Senkoh:UpdatePlayerPower(powerType)
...
function Senkoh:RefreshChargedComboPoints()
...
function Senkoh:RefreshAllPlayerPowers()
    state.power = {}
...
function Senkoh:RefreshAssistedCombatSuggestion()
...
function Senkoh:RefreshGroupTypeState()
    local index = 0
...
function Senkoh:RefreshGroupCountState()
    local count = GetNumGroupMembers()
    state.groupCount = count / 255 or 0
    self:UpdateStateBlock("状态", "队伍人数")
end

function Senkoh:UpdateHeroTalent()
...
function Senkoh:RefreshPlayerBars()
    local blocks = self.blocks
...
function Senkoh:RefreshPlayerConfigStateBlocks()
...
function Senkoh:HookChatFrameEditBox()
...
function Senkoh:CacheCollectedMountSpells()
    wipe(mounts)
...
function Senkoh:SetMountSpellCasting(spellID, isCasting)
    if isCasting then
...
function Senkoh:PreviousSkill(spellId)
    local spellIndex = spellsList[spellId] and spellsList[spellId].index or 0
...
function Senkoh:UpdatePlayerStagger()
    local unit = "player"
...
function Senkoh:UpdateHolyArmaments(spellID) -- 神圣军备
...
function Senkoh:UpdatePlayerForbearance() -- 25771 自律
...
function Senkoh:UpdateVampiricStrike(spellID) -- 吸血鬼打击
...
function Senkoh:UpdateBoilingPoint(spellID) -- 沸点
...
function Senkoh:RecordKnightSpellState(spellID)
...
function Senkoh:RefreshActiveKnightCount()
    state.knightCount = GetActiveKnightsCount() / 255
    self:UpdateStateBlock("特殊", "天启骑士数量")
end

function Senkoh:UpdateReaverGlaive(spellID) -- 收割者战刃
...
function Senkoh:UpdateHeroicStrike(spellID) -- 英勇打击
...
function Senkoh:UpdateActiveTotemRemainingTime(spellID)
