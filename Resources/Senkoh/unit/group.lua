...
local roleMap = Senkoh.roleMap
local groupHealthCurves = Senkoh.groupHealthCurves
...
function Senkoh:IterateGroupMembers(reversed, forceParty)
...
function Senkoh:RefreshGroupMemberHealth(unit)
...
function Senkoh:RefreshGroupMemberValidity(unit)
    local obj = self.group[unit]
    if not obj then return end
    obj.valid = not obj.isDead and obj.canAssist and obj.inSight
end

function Senkoh:RefreshNextGroupMemberState()
    local blocks = self.blocks
    local group = self.group
...
function Senkoh:RefreshGroupMemberDeath(unitOrGuid, source)
    local group = self.group
    if source == "guid" then
...
function Senkoh:MarkGroupMemberTemporarilyOutOfSight(unit)
    local obj = self.group[unit]
    if not obj then return end
    obj.inSight = false
    ...
    obj.inSightTimer = C_Timer.NewTimer(1.5, function()
        obj.inSight = true
        obj.inSightTimer = nil
        Senkoh:RefreshGroupMemberValidity(unit)
    end)
    self:RefreshGroupMemberValidity(unit)
end

function Senkoh:RecordIncomingHealEstimate(spellID)
...
function Senkoh:ClearIncomingHealEstimates()
...
function Senkoh:ClearGroupStateBlocks()
    local blocks = self.blocks
...
function Senkoh:RebuildGroupRoster()
