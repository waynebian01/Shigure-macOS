...
local function DebugPrintNewSpellEntry(spellID)
    if succSpells[spellID] or Senkoh.spellsList[spellID] then return end
...
Senkoh.DebugPrintNewSpellEntry = DebugPrintNewSpellEntry
Senkoh.DebugPrintSpellBlockLine = DebugPrintSpellBlockLine
...
function Senkoh:IsSpellKnown(spellID)
...
function Senkoh:ClearInsertSpell()
...
function Senkoh:SetInsertSpell(index, spellName, spellId)
    if insertSpellTimer then
        insertSpellTimer:Cancel()
        insertSpellTimer = nil
    end
    ...
    self:UpdateStateBlock("状态", "插入法术")
    local msg = "|cff00ff00[Senkoh]|r 插入法术: |cff00ff00" .. (spellName or "?") .. "|r"
    ...
    insertSpellTimer = C_Timer.NewTimer(1.5, function()
        ...
        insertSpellIndex = nil
        state.insertSpell = 0
        Senkoh:UpdateStateBlock("状态", "插入法术")
    end)
end

function Senkoh:UpdateInsertSpellBySuccess(spellID)
...
function Senkoh:ClearInsertItem()
...
function Senkoh:SetInsertItem(index, itemName, itemId)
    if insertItemTimer then
        insertItemTimer:Cancel()
        insertItemTimer = nil
    end
    ...
    self:UpdateStateBlock("状态", "插入物品")
    local msg = "|cff00ff00[Senkoh]|r 插入物品: |cff00ff00" .. (itemName or "?") .. "|r"
    ...
    insertItemTimer = C_Timer.NewTimer(1.5, function()
        ...
        insertItemId = nil
        state.insertItem = 0
        Senkoh:UpdateStateBlock("状态", "插入物品")
    end)
end

function Senkoh:UpdateInsertItemBySuccess(spellID)
...
local function UpdateCooldownSpellKnown()
    spells = {}
    if not Senkoh.blocks or not Senkoh.blocks.spells then return end
    C_Timer.After(1, function()
        local blocks = Senkoh.blocks
        if not blocks or not blocks.spells then return end
        for spellID, info in pairs(blocks.spells) do
            local isKnown = Senkoh:IsSpellKnown(spellID)
            ...
            if isKnown or info.forcedKnown then
                ...
                if info.index then
                    Senkoh:CreateTexture(info.index, 1)
                end
                if info.charge then
                    Senkoh:CreateTexture(info.charge, 1)
                end
            end
        end
    end)
...
function Senkoh:UpdateSpellKnown()
...
function Senkoh:UpdateSpellCooldown()
...
function Senkoh:GetItemRemainingTime(itemID, isEquipped)
    local itemExists
    if isEquipped then
...
function Senkoh:UpdateItemCooldown()
