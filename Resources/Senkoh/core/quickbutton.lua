--[[
摘要：
    Senkoh 快捷控件：panel.lua 蓝本——barLayer 爆发计时条（BurstTime 时长驱动，
...
描述：
    ...
    与 /se timer [on|off] 共用 StartTimer/StopTimer。
    爆发状态模型：Senkoh.BurstTime（GetTime 纪元时间戳）为命名空间级唯一真相、初始 0 = 未
    开启；独立匿名 driver frame（UIParent 顶层常驻，与面板可见性解耦——/se hide 隐藏面板期间
...
主要变量信息：
    ...
    Senkoh.BurstTime：爆发计时截止时间戳（GetTime 纪元），命名空间变量与唯一真相，
        初始 0 = 未开启；barLayer 点击分派与 /se cd 命令写入，driver frame 派生 c.cooldowns
...
修改记录：
    2026-08-18：整体重写——旧单按钮 50x64 四行文字（爆/群/模/药）UI 移除，改为 panel.lua
        蓝本面板（barLayer 计时条 + 双按钮行）；爆发状态模型升级为 Senkoh.BurstTime
        时间戳驱动，独立 driver frame 派生 c.cooldowns；/se cd 命令与新模型适配
    2026-09-04：第三枚计时器按钮（左击开始 / 右击关闭，超过 255 秒自动关闭）；面板加宽至 180；
        与 /se timer [on|off] 共用 StartTimer/StopTimer
--]]

local addon, ns = ... -- 保持 Senkoh 文件惯例，本文件不引用 addon/ns
...
-- 命名空间防御与爆发状态唯一真相：Senkoh.BurstTime 时间戳（GetTime 纪元），初始 0 = 未开启，
-- driver frame 首帧派生 c.cooldowns=0（旧档 cooldowns=1 不迁移，重启归零、首帧静默纠正）
Senkoh = Senkoh or {}
Senkoh.BurstTime = 0

-- 独立 driver frame：匿名常驻 Frame（UIParent 顶层，非面板子元素，与面板可见性解耦——/se hide
...
driverFrame:SetScript("OnUpdate", function()
    local cd = (Senkoh.BurstTime > GetTime()) and 1 or 0
    local c = Senkoh:GetCharConfig()
...
function Senkoh:UpdateQuickToggleVisibility()
...
function Senkoh:HideQuickToggleButton()
    local c = self:GetCharConfig()
    if not c then return end
    c.quickButtonShow = false
    self:UpdateQuickToggleVisibility()
    print("|cff00ff00[Senkoh]|r 快捷控件已隐藏。")
end

function Senkoh:ShowQuickToggleButton()
    local c = self:GetCharConfig()
    if not c then return end
    c.quickButtonShow = true
    self:UpdateQuickToggleVisibility()
    print("|cff00ff00[Senkoh]|r 快捷控件已显示。")
end

-- 计时器按钮显示态：未运行显示「计时器」，运行中显示已过秒数（与 StartTimer/StopTimer 共用）
function Senkoh:RefreshTimerAppearance()
    local f = self.quickToggleFrame
...
function Senkoh:RefreshQuickToggleAppearance()
    local f = self.quickToggleFrame
...
function Senkoh:InitQuickToggleButton()
    ...
    local c = Senkoh:GetCharConfig()
    -- 面板根框体：匿名（全局命名 SenkohBurstPanel 不再创建），可拖动、位置记忆
    ...
    -- 悬停 Tooltip（照蓝本三行文案）：标题行 SetText 绿（Senkoh）、三行按键说明 AddLine 灰，
    ...
    local function ShowBurstTooltip()
        ...
        GameTooltip:SetText("Senkoh", 0, 1, 0.6, 1, true)
    ...
    barLayer:SetScript("OnUpdate", function(self)
        local remaining = Senkoh.BurstTime - GetTime()
    ...
    -- 中键 +3600 秒长计时 / 其余按键维持 +15，写 Senkoh.BurstTime 并同步写 c.cooldowns（静默）；
    ...
    barLayer:SetScript("OnMouseUp", function(self, button)
        ...
        if math.abs(x - pressX) < CLICK_THRESHOLD and math.abs(y - pressY) < CLICK_THRESHOLD then
            if button == "LeftButton" then
                Senkoh.BurstTime = GetTime() + 15
            elseif button == "RightButton" then
                Senkoh.BurstTime = GetTime() - 1
            elseif button == "MiddleButton" then
                Senkoh.BurstTime = GetTime() + 3600
            else
                Senkoh.BurstTime = GetTime() + 15 -- Button4/Button5 等未请求变更的按键维持现状
            end
            local cc = Senkoh:GetCharConfig()
            if cc then
                cc.cooldowns = (Senkoh.BurstTime > GetTime()) and 1 or 0
    ...
    local buttonDefs = {
        {
            ...
            applyClick = function(c)
                ...
                if Senkoh.SwitchAoeMode then Senkoh:SwitchAoeMode() end
            end,
        },
        {
            ...
            applyClick = function(c)
                ...
                if Senkoh.SwitchPotion then Senkoh:SwitchPotion() end
            end,
        },
    }
    ...
    timerButton:SetScript("OnClick", function(_, mouseButton)
        if mouseButton == "LeftButton" then
            if Senkoh.StartTimer then Senkoh:StartTimer() end
        elseif mouseButton == "RightButton" then
            if Senkoh.StopTimer then Senkoh:StopTimer() end
        end
    end)
