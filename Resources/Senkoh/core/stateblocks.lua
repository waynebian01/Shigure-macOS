...
function Senkoh:GetUnitCastPixel(unit, mode)
...
--- mode: "cast" | "channel"
function Senkoh:GetUnitInterruptiblePixel(unit, mode)
    if mode == "channel" then
        local channel = UnitChannelDuration(unit)
        if not channel then return 0 end
...
for powerType, powerName in pairs(Senkoh.powerNameMap) do
...
function Senkoh:UpdateStateBlock(category, name)
...
function Senkoh:UpdateBareStateBlock(name, categories)
