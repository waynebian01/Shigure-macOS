...
-- 主色条（SenkohColorBars / CreateTexture）
...
-- 横向条（SenkohCountBars：计数条 + 光环层数条，BAR_END_COLOR 收尾）
...
local colorBars = CreateFrame("Frame", "SenkohColorBars", UIParent)
...
function Senkoh:CreateTexture(i, b)
...
function Senkoh:ClearAllTextures()
...
function Senkoh:UpdateBlockLayout(usedCount)
...
local countBars = CreateFrame("Frame", "SenkohCountBars", UIParent)
...
function Senkoh:CreateAutoLayoutBar(valueType, minValue, maxValue, spellId)
    ...
    local startIndex = ReserveHorizontalBarUnits(maxValue, "警告: Senkoh_CountBars 空间不足!")
    ...
    local function Refresh()
        ...
        elseif valueType == "charge" then
            ...
            if charges and Senkoh:IsSpellKnown(spellId) then
...
function Senkoh:ClearAllSenkohBars()
    for _, bar in ipairs(createdBars) do
    ...
    if Senkoh.ReleasePlayerAuraContainers then
        Senkoh:ReleasePlayerAuraContainers()
    end
    if Senkoh.ReleaseGroupAuraContainers then
        Senkoh:ReleaseGroupAuraContainers()
    end
    if Senkoh.ClearGroupHealAbsorbBars then
        Senkoh:ClearGroupHealAbsorbBars()
    end
end
...
local healAbsorbBars = CreateFrame("Frame", "SenkohHealAbsorbBars", UIParent)
...
function Senkoh:UpdateGroupHealAbsorbBar(unit)
...
function Senkoh:ClearGroupHealAbsorbBars()
...
function Senkoh:RefreshGroupHealAbsorbBars()
...
healAbsorbBars:SetScript("OnEvent", function(_, _, unit)
    if unit and healAbsorbUnitToSlot[unit] then
        Senkoh:UpdateGroupHealAbsorbBar(unit)
    end
end)
...
local function CollectAuraSpellSlots(unitFilter)
    local slots = {}
    local auras = Senkoh.blocks and Senkoh.blocks.auras
    if not auras then
        return slots
    end
...
local function AddDurationAuraSlotPair(container, slotKeyPrefix, filter, includeSpellIDs, index)
    ...
    container.senkohAuraSlots = container.senkohAuraSlots or {}
    tinsert(container.senkohAuraSlots, {
...
local function ApplyUnitAuraReactionFilters(container, unit)
    local slots = container and container.senkohAuraSlots
    if not slots then
        return
    end
...
local function RebindContainerSpellFilters(container, unit)
    ...
    local bindUnit = unit or container.senkohUnit
    KickAuraContainer(container, bindUnit)

    if container.senkohAuraSlots then
        ApplyUnitAuraReactionFilters(container, bindUnit or "player")
    end
    if container.senkohBarSlots then
        table.sort(container.senkohBarSlots, function(a, b)
            return (a.index or 0) < (b.index or 0)
        end)
        for _, slot in ipairs(container.senkohBarSlots) do
    ...
    if container.senkohSpellIdSlots then
        for _, slot in ipairs(container.senkohSpellIdSlots) do
            container:SetAuraSlotCandidateFilters(slot.key, AuraSlotFilters(slot.includeSpellIDs))
        end
    end
    if container.senkohDispelSlot then
        local dispel = container.senkohDispelSlot
        container:SetAuraSlotCandidateFilters(dispel.key, {
            includeDispelTypes = dispel.includeDispelTypes,
        })
    end
    if container.senkohUnitDispelSlots then
        local canAttack = bindUnit and UnitCanAttack("player", bindUnit)
        local canAssist = bindUnit and UnitCanAssist("player", bindUnit)
        for _, slot in ipairs(container.senkohUnitDispelSlots) do
            ...
            local source = slot.reaction == "enemy"
                and Senkoh.includeOffensiveDispelTypes
                or Senkoh.includeDispelTypes
...
local function AuraBarSlotsMatch(container, appSlots)
    local slots = container and container.senkohBarSlots
...
function Senkoh:ReleaseUnitAuraContainers()
    for _, key in pairs(UNIT_AURA_CONTAINER_KEYS) do
        ReleaseFrame(Senkoh[key])
        Senkoh[key] = nil
    end
...
-- 兼容旧名
function Senkoh:ReleasePlayerAuraContainers()
    self:ReleaseUnitAuraContainers()
end

local function GetUnitDispelStateIndex(unit)
    local stateBlocks = Senkoh.blocks and Senkoh.blocks.state
...
local function AddUnitDispelSlots(container, unit, index)
    ...
    container.senkohUnitDispelSlots = slots
end
...
local function CreateUnitAuraDurationSlots(unit, spellSlots, dispelIndex)
    ...
    local frameName = "Senkoh" .. unit:gsub("^%l", string.upper) .. "AuraDurationSlots"
...
function Senkoh:UpdateUnitAuraContainer(unit)
    local key = UNIT_AURA_CONTAINER_KEYS[unit]
    if not key then
        return
    end
    RebindContainerSpellFilters(Senkoh[key], unit)
    RebindContainerSpellFilters(Senkoh[UNIT_AURA_BAR_CONTAINER_KEYS[unit]], unit)
end

-- 兼容旧名
function Senkoh:RefreshPlayerAuraContainers()
    self:RefreshUnitAuraContainers()
end

--- 在计数条之后排布层数条，最后放置 BAR_END_COLOR
--- 按 auras 索引升序固定条序；若容器已存在但槽位集合变化则整表重建
function Senkoh:LayoutAuraApplicationBars()
    local slotsByUnit = {}
    ...
    for _, unit in ipairs(AURA_BAR_UNIT_ORDER) do
        ...
        local hasContainer = Senkoh[key] ~= nil
        if not AuraBarSlotsMatch(Senkoh[key], appSlots) then
    ...
    for _, unit in ipairs(AURA_BAR_UNIT_ORDER) do
        ...
        EnsureAuraContainerLoaded()
            ...
            local key = UNIT_AURA_BAR_CONTAINER_KEYS[unit]
            local frameName = "Senkoh" .. unit:gsub("^%l", string.upper) .. "AuraBarSlots"
            ...
            barSlots.senkohBarSlots = {}
            barSlots.senkohUnit = unit

            for _, info in ipairs(appSlots) do
                local startIndex = ReserveHorizontalBarUnits(
                    info.maxApps,
                    "警告: Senkoh_CountBars 光环层数条空间不足!"
                )
                if not startIndex then
                    break
                end
                ...
                tinsert(barSlots.senkohBarSlots, slotInfo)
            end
            Senkoh[key] = barSlots
...
local function CopyIncludeDispelTypes()
    local src = Senkoh.includeDispelTypes
...
function Senkoh:ReleaseGroupAuraContainers()
...
local function CreateGroupMemberRoleOverlayContainer(memberIndex, groups)
    ...
    local container = CreateFrame("AuraContainer", "SenkohGroupRoleOverlaySlots_" .. memberIndex, UIParent,
                                  "CustomAuraContainerTemplate")
...
local function CreateGroupMemberAuraContainer(memberIndex, groups, auraDefs, includeDispelTypes)
    ...
    local container = CreateFrame("AuraContainer", "SenkohGroupAuraSlots_" .. memberIndex, UIParent,
                                  "CustomAuraContainerTemplate")
    container:SetPoint("TOPLEFT", UIParent, "TOPLEFT", 0, 0)
    container:SetEnabled(true)
    container:SetFrameStrata(AURA_DURATION_STRATA)
...
function Senkoh:RefreshGroupAuraContainers()
    local groups = Senkoh.blocks and Senkoh.blocks.groups
    ...
    local groupList = Senkoh.groupList or {}
    local group = Senkoh.group or {}
...
function Senkoh:RebindAuraSpellFilters()
    ...
    for _, unit in ipairs(AURA_BAR_UNIT_ORDER) do
        local key = UNIT_AURA_BAR_CONTAINER_KEYS[unit]
        local barContainer = Senkoh[key]
        if barContainer and barContainer.senkohBarSlots then
            local appSlots = CollectAuraApplicationSlots(unit)
            for i, slot in ipairs(barContainer.senkohBarSlots) do
            ...
            table.sort(barContainer.senkohBarSlots, function(a, b)
                return (a.index or 0) < (b.index or 0)
            end)
            for _, slot in ipairs(barContainer.senkohBarSlots) do
    ...
    for _, memberIndex in ipairs(groupIndices) do
        local container = groupAuraContainers[memberIndex]
        RebindContainerSpellFilters(container, container.senkohUnit)
    end
