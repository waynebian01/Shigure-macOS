...
local eventFrame = CreateFrame("Frame", "SenkohEventFrame")

--- 与 AceEvent 一致：回调为 addon[event](addon, event, ...)
function Senkoh:RegisterEvent(event)
    eventFrame:RegisterEvent(event)
end

function Senkoh:UnregisterEvent(event)
    eventFrame:UnregisterEvent(event)
end

function Senkoh:UnregisterAllEvents()
...
--- 兼容原 AceDB-3.0 的 SenkohADB 布局（char / profiles / profileKeys）
local function InitDB()
    if type(SenkohADB) ~= "table" then
        SenkohADB = {}
    end
    local sv = SenkohADB
    sv.char = sv.char or {}
    ...
    CopyDefaults(char, Senkoh.defaults.char)
    ...
    CopyDefaults(profile, Senkoh.defaults.profile)
...
function Senkoh:OnInitialize()
    InitDB()

    SLASH_SENKOH1 = "/se"
    SLASH_SENKOH2 = "/senkoh"
    SlashCmdList["SENKOH"] = function(msg, editbox)
        Senkoh:SlashCommand(msg, editbox)
    end

    self:InitializeCharacterState()
end

function Senkoh:OnEnable()
...
Senkoh.blocks = {}
Senkoh.target = CreateUnitState()
Senkoh.focus = CreateUnitState()
Senkoh.mouseover = CreateUnitState()
Senkoh.pet = CreateUnitState()
Senkoh.boss = {}
for index = 1, 5 do
    Senkoh.boss["boss" .. index] = CreateUnitState()
end
Senkoh.nameplate = {}
Senkoh.group = {}
Senkoh.groupList = {}
...
eventFrame:SetScript("OnEvent", function(_, event, ...)
    ...
    local handler = Senkoh[event]
    if handler then
        handler(Senkoh, event, ...)
    end
end)
